...
with `.exe` added for Windows. The MVP publishes these two binaries:
...
two binaries listed above. Before running a downloaded binary, download
...
GitHub Actions builds the two binaries on their target runners,
...
the three named assets with generated release notes.
